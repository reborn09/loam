...
// local headers
#include "loam_cmu/parameterReader.h"
...
PARAMETER_READER parameterReader("/root/catkin_SLAM/src/loam_cmu/parameters/poseGraphPara.txt");
...
////////////////////////////////////////////////////////////////////////////////
Eigen::Matrix4f NDTAlignment(const int srcInd, const int dstInd, const bool setInitGuess)
{
...
/// \return transform matrix from source cloud to target cloud
////////////////////////////////////////////////////////////////////////////////
Eigen::Matrix4f ICPAlignment(const int srcInd, const int dstInd)
{
  pcl::PointCloud<pcl::PointXYZI>::Ptr srcCloud(new pcl::PointCloud<pcl::PointXYZI>);
  pcl::PointCloud<pcl::PointXYZI>::Ptr referenceCloud(new pcl::PointCloud<pcl::PointXYZI>);
  srcCloud->clear();
  ...
  while(status)
  {
        ...
        && fabs(timeLaserCloudFlat-timeLaserPose) < 0.005 )
    {
      ...
      *pcSurfPtr = tmpCloudSurf;
      ...
      std::cout << "update graph time: "<<(t_end.tv_sec - t_start.tv_sec)*1000.0
    ...
    if(g2oOpen && !loopClosed && frameInd > 0 && counter >= OptimizationWait)
    {
      ...
      // alignment
      Eigen::Matrix4f loopError = ICPAlignment(frameInd, 0);
